    ...
    @IBAction func signIp(_ sender: Any) {
        ...
        if l.isEmpty || p.isEmpty {
            ...
            if user.isBlackList == true {
                let alert = UIAlertController(title: "Attention", message:"You are blacklisted", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "Ok", style: .default))
                present(alert, animated: false, completion: nil)
            }
            else if l == user.login && p == user.password{
                if let newViewController = storyboard? .instantiateViewController(withIdentifier: "menuViewController") as? MenuViewController{
